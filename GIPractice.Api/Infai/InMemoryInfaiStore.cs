...
public sealed class InMemoryInfaiStore : IInfaiStore
{
    ...
    {
        public InfaiReportId Id { get; init; }
        ...
        public InfaiReportStatus Status { get; set; }
    ...
    public Task<PagedResultDto<InfaiReportListItemDto>> SearchAsync(InfaiReportSearchRequestDto request, CancellationToken ct)
    {
        lock (_lock)
        {
            ...
            var items = q.Skip((page - 1) * pageSize).Take(pageSize)
                .Select(x => new InfaiReportListItemDto(
                    x.Id, x.PatientId, x.EndoscopyId,
                    x.SentUtc, x.ReceivedUtc, x.IsUrgent, x.Status))
                .ToList();

            return Task.FromResult(new PagedResultDto<InfaiReportListItemDto>(items, total, page, pageSize));
        }
    }

    public Task<InfaiReportDto?> GetAsync(InfaiReportId id, CancellationToken ct)
    {
        lock (_lock)
        {
            var x = _rows.FirstOrDefault(r => r.Id.Equals(id));
            if (x is null) return Task.FromResult<InfaiReportDto?>(null);

            return Task.FromResult<InfaiReportDto?>(new InfaiReportDto(
                x.Id, x.PatientId, x.EndoscopyId,
    ...
    public Task<ResultDto<InfaiReportId>> CreateAsync(InfaiReportUpsertRequestDto request, CancellationToken ct)
    {
        lock (_lock)
        {
            var id = new InfaiReportId(_nextId++);
            ...
            _rows.Add(row);
            return Task.FromResult(ResultDto<InfaiReportId>.Ok(id));
        }
    }

    public Task<ResultDto<bool>> UpdateAsync(InfaiReportUpsertRequestDto request, CancellationToken ct)
    {
        ...
        lock (_lock)
        {
            ...
            if (row is null)
                return Task.FromResult(ResultDto<bool>.Fail("not_found", "INFAI report not found."));
    ...
    public Task<ResultDto<bool>> DeleteAsync(InfaiReportId id, CancellationToken ct)
    {
        lock (_lock)
        {
